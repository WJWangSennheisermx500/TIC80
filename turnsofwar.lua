...
+4,0)end,30,ju),jD(ku(sq),jD(kn
(sq.r*16+8,sq.s*16+8),km("-"..sr
,sq.r*16+8-5,sq.s*16,6))),ss)end
function kr(sz,sA,sB)return jD(ku
(sz),jD(kn(sz.r*16+8,sz.s*16+8)
,km(sA,sz.r*16+8-5,sz.s*16,sB))
)end function ks(sC,sD,sE,sF,sG)local
sH=sD-sC.r local sI=sE-sC.s local sJ
=nil for sK=1,#sF do local sL=sF[sK]
local sM if sD==sL.r and sE==sL.s then
sM=4 else sM=2 end local sN=kr(sL,"-"
..sM,6)if sJ==nil then sJ=sN else sJ
=jD(sJ,sN)end end return jE(jC(function
(sO,sP,sQ)spr(304,16*(sC.r+sP*sH
)+4,16*(sC.s+sP*sI)+4,0)end,30,
ju),sJ,sG)end function kt(sR,sS,sT
,sU,sW)local sX=sS.r-sR.r local sY
=sS.s-sR.s return jE(jC(function(
sZ,ta,tb)spr(257,16*(sR.r+ta*sX
)+4,16*(sR.s+ta*sY)+4,0)end,30,
ju,function()if sU then sfx(4,60,40
)else sfx(11,50,40)end end),kr(sS
,sT,11),sW)end function ku(tc)return
ko(tc,function(td,te,tf)td.ax=jn
(2)-1 td.ay=jn(2)-1 end,30,ju)end
function kw(tg,th,ti,tj)local tk=
th.r-tg.r local tl=th.s-tg.s return
jE(ko(tg,function(tm,tn,to)tm.ax
=tn*14*tk tm.ay=tn*14*tl end,30,jw
,function()sfx(0,20,30)end),jD(
ko(tg,function(tp,tq,tr)tp.ax=(
14-tq*14)*tk tp.ay=(14-tq*14)*tl
end,30,ju),jD(ku(th),jD(kn(th.r
*16+8,th.s*16+8),km("-"..ti,th.
r*16+8-5,th.s*16,6)))),tj)end function
kx(ts,tt,tu,tw)local tx=tt-ts.r local
ty=tu-ts.s return ko(ts,function(
tz,tA,tB)tz.ax=tA*16*tx tz.ay=tA
*16*ty end,90,jz,tw)end function ky
(tC,tD)if tD.y then tC.y[tD]=tD.y end
if tC.x then local tE=tC.x[tD]if tE and
tE>0 then tC.x[tD]=tE-1 return end end
tC:A(kS.aT)end function kz(tF,tG,
tH)return math.abs(tF.r-tG)+math
.abs(tF.s-tH)end function kA(tI)local
tJ={}for tK,tL in pairs(tI)do for tM
=1,#tL do local tN=tL[tM]if not tJ[tN
]then tJ[tN]={}end table.insert(tJ
[tN],tK)end end return tJ end function
kB()end function kC(tO,tP,tQ)local
...
tR end function kD(tX,tY)local tZ=ro
:N(tX,tY)if tZ and tZ.t<0 then return
false end return jM(nil,nil,tX,tY)
end function kE(ua,ub,uc)iX={}local
...
]=uo end end return 1000 end function
kF(up,uq)local ur={}for us=1,#uq do
...
ur[1].s end function kG(uA)local uB
=uA.w[kT.bz]local uC,uD=kF(uA,uB
)kT.bz:by(uA,uC,uD)end function kH
...
(uI,uM,uN)end function kJ(uS)for uT
...
end function kK()local uZ=ro:U(ro.
...
end function kL(wi)for wj=1,#ro.K do
local wk=ro.K[wj]if wk.t==wi then jH
(wk)end end end function kM()for wl,
...
end end function kQ(wy,wz)ky(wz,wy
)kP()end function kR(wA,wB,wC)wA.
r=wB wA.s=wC end kS={aT={aU="Spent"
,aX=272,aY=1,aZ="Unit already moved this turn"
},bb={aU="Hexen",aX=273,aY=4,aZ
="Does 1 less damage"},be={aU="Morale"
,aX=320,bg=6,aY=-1,aZ="Receive 1 less damage on any attack"
,bi=true},bj={aU="Ensnared",aX=
288,aY=3,aZ="Unit will not be able to move"
},bm={aU="Focused",aX=289,aY=4,
aZ="Range of attacks increased by 1"
},bp={aU="Furious",aX=321,aY=4,
aZ="Attack dmg increased by 1"}
}kT={bs={aU="Move",bu=jp(jN,function
(wD,wE,wF)return not wD:E(kS.bj)end
,jL,jM),bw=448,aZ="Move unit to nearby cell"
,by=function(wG,wH,wI,wJ)sfx(10
,15+jn(25),20)je=kp(wH,wI,wJ,function
()kR(wH,wI,wJ)kQ(wG,wH)end)end}
,bz={aU="Attack",bu=jp(jS,jL),bw
=449,aZ="Attack a nearby unit",
by=function(wK,wL,wM,wN)local wO
=ro:N(wM,wN)local wP=jn(wL.q.cy)
if wL:E(kS.bb)then wP=wP-1 end if wO
:E(kS.be)then wP=wP-1 end if wL:E(kS
.bp)then wP=wP+1 end if wP<0 then wP=
0 end je=kw(wL,wO,wP,function()wO
:B(wP)kQ(wK,wL)end)end},bC={aU=
"Ranged",bu=jp(jR,jS),bw=449,aZ
="Attack an enemy at distance",
...
.t)kQ(xF,xG)end)end},ca={aU="Recall"
...
(xP,xQ,xR)return true end},ce={aU
...
,"withstand the assault","at the village!"
,"","Positive spells usually","have green circle target."
},ec={kY.cX,5,4,kY.cu,5,5,kY.cu
,4,4,kY.cu,6,4,kY.cu,5,3,},ed={
...
,"","Try to keep skeletons busy"
,"while the elves shoot them!",
},ec={kY.dd,1,3,kY.cu,0,4,kY.cC
,1,5,kY.cC,1,2,},ed={kY.dx,10,6
,kY.dx,10,5,kY.dx,10,4,kY.dx,10
,3,kY.dx,10,2,kY.dx,9,6,kY.dx,8
,4,kY.dx,8,5,kY.dx,8,3,kY.dx,9,
2,},L={[-2]=function()kU({"\"Why are they attacking?"
...
,"is dangerous. Luckly,","one of the Slug people"
,"accepted to aid us","with the run!"
},ec={kY.cC,1,0,kY.dn,0,0,kY.cX
,1,3,kY.cu,8,7,kY.dF,0,1,kY.cu,
7,7,},ed={kY.dB,10,3,kY.dH,7,0,
kY.dH,8,0,kY.dD,6,3,kY.dD,7,3,kY
...
,"tough!","","A solitary monk living there"
,"can provide some help!"},ec={
...
,"carefully and exploit the","wizard!"
},ec={kY.cX,1,3,kY.dF,0,4,kY.dq
...
,"turns as he's indispensable",
"to defeat all those enemies!"}
...
,3,2,kY.cC,0,3,kY.dn,0,4,kY.dj,
2,4,kY.dl,1,4,kY.cX,1,3,kY.dd,1
...
,"different kind of monks","dwells. The fighting kind!"
,"","Beat them to earn their","respect and loyalty!"
...
,"trying to surround us.","Teach em a lesson."
},ec={kY.cu,4,2,kY.du,5,4,kY.du
...
,"monsters, and if we can find"
,"him maybe we can hire him!"},
ec={kY.cu,1,3,kY.du,1,4,kY.dF,0
,4,kY.dl,0,3,kY.cC,0,2,kY.cX,0,
5,kY.dq,1,6,},ed={kY.db,9,3,kY.
...
,"It looks like the castle is",
"sieged too!"},ec={kY.cu,1,3,kY
.cz,1,4,kY.cu,1,5,kY.dj,0,4,kY.
...
,"","We must find where they","come from!"
},ec={kY.dh,1,3,kY.cz,1,4,kY.cu
,1,5,kY.cu,0,5,kY.cu,0,6,kY.dq,
0,3,},ed={kY.db,9,3,kY.cZ,8,4,kY
.db,8,2,kY.cZ,10,3,kY.cZ,9,4,},
...
,"well where she can perform a"
,"divination and locate the","monsters source."
,"","We're kind of surrounded here,"
,"Power up those elves and","resist the assault!"
},ec={kY.dh,0,0,kY.cC,1,0,kY.cu
,1,2,kY.cC,0,2,kY.dn,1,1,kY.ds,
...
"the ritual!","","Looks like the forces of"
,"Evil got wind of our plans","becouse monsters are"
,"literally pouring in!"},ec={kY
.dh,0,0,kY.cC,0,3,kY.cu,1,2,kY.
cC,0,2,kY.dn,1,1,kY.ds,0,1,},ed
={kY.cZ,8,7},hN=function()local
yS=ro:S(kY.dx,10,5,1)if yS then yS
.u[kS.aT]=1 end local yS=ro:S(kY.dx
,4,7,1)if yS then yS.u[kS.aT]=1 end
local yS=ro:S(kY.dx,5,0,1)if yS then
yS.u[kS.aT]=1 end end,L={[jr(7,4)
]=function()local yT=ro:N(7,4)if
yT.q==kY.ds then kU({"ABRA CADABRA!"
,"The witch performs a dark","divination ritual!"
,"She observe a portal","not too far away,"
,"from where the monsters","are coming..."
,"","We must close it!","","The energy blast from"
,"the ritual kills all","enemies!"
},kN,ji.e)else kU({"ARGH!!","\"Only a witch can withstand"
...
,"just spit out the strongest",
"of all enemies, the Archdemon!"
,"","We must kill all monsters and"
,"destroy the portal! While it is"
,"open, more skeletons will","enter!"
,"","Good luck, for our freedom!"
},ec={kY.dh,0,0,kY.cC,0,3,kY.cu
,1,2,kY.cC,0,2,kY.dn,1,1,kY.ds,
0,1,kY.cz,0,4,kY.dl,0,5,kY.dF,2
,0,kY.dj,1,0,kY.cX,1,4,kY.dd,1,
3,},ed={kY.dN,10,7,kY.dJ,10,6,kY
.db,9,7,kY.dH,8,6,kY.dH,9,5,kY.
cZ,6,4,kY.cZ,5,5,kY.dz,9,3,kY.dP
,9,6},hN=function()if ro:W(kY.dP
)>0 then if ro:W(kY.dx)<10 then local
yX=ro:S(kY.dx,10,5,1)if yX then yX
.u[kS.aT]=1 end local yX=ro:S(kY.dx
...
,"celebrations concluded, ","peace finally return to the"
,"lands."},ec={kY.dh,0,0,kY.cE,
5,6,kY.cE,6,4,kY.cE,7,2,kY.cu,0
,2,kY.cu,1,2,},ed={},hN=function
()if not ro.msgdone then ro.msgdone
=true kU({"The Game is Over","This is a game over screen."
,"You defeated the evil portal"
,"","You can move around the","farmers but they're just"
,"there to signify peace. There"
,"are no more monsters to kill."
,"","If you liked this game, please"
,"leave a thumbs up on the tic80"
,"website. Thanks :)"},function
()kU({"A game by MSX","Reach me on TIC80 discord or"
,"at msx80 @ twitter","","Graphics by: "
